function [ accu_F_U,accu_F_T ] = Func_random( X_L,Y_L,X_U,Y_U,X_T,Y_T,Number_Iter )
   ...
   %% 开始主动在线学习
    ...
    for k=1:Number_Iter
     %   Hx_min=1e300;
        ...
        index=randi(u_train+u_test,1);
          
          % 选择数据给人标注，获得标签后加入到标注训练集，更新下一轮数据集
          if(index>u_train)
             % 从测试集中加入到标注训练集中
              example=Set_test_U(index-u_train);
